...
export function WorkoutLogger({
  ...
  const [activeExerciseTabs, setActiveExerciseTabs] = useState<Record<string, 'sets' | 'history'>>({});

  const getActiveTab = (exerciseId: string) => activeExerciseTabs[exerciseId] || 'sets';
  const setActiveTab = (exerciseId: string, tab: 'sets' | 'history') => {
    setActiveExerciseTabs(prev => ({ ...prev, [exerciseId]: tab }));
  };
              ...
              return (
                ...
                >
                  <Card className="p-0 overflow-hidden">
                    <div className="p-4 flex items-center justify-between bg-bg-tertiary/20 border-b border-border-primary">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl bg-accent-primary/10 flex items-center justify-center text-accent-secondary">
                          <Trophy size={20} />
                        </div>
                        <div>
                      ...
                      <Button
                        ...
                        className="p-2 h-auto text-text-subtle hover:text-danger"
                      >
                        <Trash2 size={18} />
                      </Button>
                    </div>
                    ...
                    <div className="flex border-b border-border-primary">
                      <button
                        onClick={() => setActiveTab(workoutEx.id, 'sets')}
                        className={`flex-1 py-3 text-sm font-bold transition-colors ${
                          getActiveTab(workoutEx.id) === 'sets'
                      ...
                      <button
                        onClick={() => setActiveTab(workoutEx.id, 'history')}
                        className={`flex-1 py-3 text-sm font-bold transition-colors ${
                          getActiveTab(workoutEx.id) === 'history'
                            ? 'text-accent-secondary border-b-2 border-accent-primary'
                            : 'text-text-dim hover:text-text-secondary'
                        }`}
                      >
                        ...
                        <>
                          {workoutEx.sets.map((set, idx) => (
                            <div key={set.id} className="flex items-end gap-3 group">
                              <div className="w-8 pb-3 text-center text-xs font-bold text-text-subtle">{idx + 1}</div>
                              <SetInputs
                                set={set}
                              ...
                              <Button
                                ...
                                className="p-2 mb-1.5 h-auto text-text-faint hover:text-danger opacity-0 group-hover:opacity-100 transition-opacity"
                              >
                                <Trash2 size={16} />
                              </Button>
                            </div>
                          ...
                          <Button 
                            ...
                            className="w-full mt-2 py-3 border-dashed bg-transparent hover:bg-bg-tertiary/50"
                          >
                            <Plus size={16} /> Add Set
                          </Button>
                        </>
                      ) : (
                        <ExerciseHistory exerciseId={workoutEx.exerciseId} />
                      )}
                    </div>
                  </Card>
                </motion.div>
