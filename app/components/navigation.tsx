...
const DEV_NAV_ITEM: NavItem = { id: 'dev', label: 'Dev', icon: Code };

export function Sidebar({ activeTab, setActiveTab }: Props) {
  const { theme, toggleTheme } = useTheme();
  const { user } = useUser();

  const navItems = user?.role === 'dev'
    ? [...BASE_NAV_ITEMS, DEV_NAV_ITEM]
    : BASE_NAV_ITEMS;
  ...
  return (
    ...
    >
      <div className="flex items-center gap-4 mb-12">
        <div
          className={`
            w-12 h-12 rounded-2xl bg-linear-to-br 
            from-accent-primary to-accent-pink 
        ...
        <span
          className={`
            ...
            bg-linear-to-r from-text-primary to-text-dim 
            tracking-tight hover:cursor-pointer
          `}
      ...
      <button
        ...
        className={`
          flex items-center gap-4 px-6 py-4 rounded-2xl 
          transition-all duration-300 font-bold 
          hover:cursor-pointer text-text-dim 
          hover:text-text-secondary hover:bg-bg-secondary mt-4
        `}
      >
    ...
    >
      {navItems.map((item) => (
        <button
          ...
          className={cn(
            `
              ...
              hover:cursor-pointer hover:text-accent-primary
            `,
            activeTab === item.id
        ...
        >
          <item.icon
            size={22}
            className={cn(activeTab === item.id && "scale-110")}
          />
          <span
            className={`
